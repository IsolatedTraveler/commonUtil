export function getCommonCombogrid(params) {
  try {
    var id = params.id;
    ...
    //校验数据有效性
    /*$("#"+id).next().children(":text").blur(function() {
        try{
        var pClosed = $("#"+id).combogrid("panel").panel("options").closed;
        firstsearch=true;
        if (pClosed) {
          var textvalue = $("#"+id).combogrid('getText');
          var combodata=$("#"+id).combogrid('grid').datagrid('getRows');
          ...
          if(setValue!=idvalue || setValue==""){
            $("#"+id).combogrid("setValue",setValue);
            if(flag==true){
              $("#"+id).combogrid("setText",textvalue);
            }
            if(required && $("#"+id).combogrid("getText")==""){
