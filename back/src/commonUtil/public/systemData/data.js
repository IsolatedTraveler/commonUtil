...
function setSystemTime() {
  let now = new Date(), date = now.addHour(1).getTime(), sd = that.commonHttppost('/zs01-xtjc/s-time', {}, { isGetUser: false }).data[0].sd
  timeDiff = {
...
export function getRyxx(ksid, ryxz, bz, ryzw) {
  return that.commonQueryAsyncHttppost_callback('/zs02-ywjc/ryxxgl/s-bmryxx', { ksid, ryxz, ryzw }).then(res => {
    return res.data
  })
}
export function faceVerify(param, options) {
  ...
  return that.commonQueryAsyncHttppost_callback(getFaceUrl(), param).then(res => {
    if (res.code == '1') {
      if (res.face_num > 0) {
