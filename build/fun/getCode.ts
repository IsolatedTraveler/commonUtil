...
function dealCode(code: string) {
  // return prettier.format(code || '', {parser: 'babel', filepath: path.resolve(__dirname, '../../.prettierrc')}).then(code => {
  return format(code || '', {
    parser: 'typescript',
    semi: true,
    trailingComma: 'none',
...
function getFile(res: string, moduleName: string, moduleFile: string, src: string, name: string, external?: string[]) {
  ...
  if (src) {
    return contantFileCode(
      src,
      res.replace(/([ \t]*\/\/[ ]*)@CODEMODULE([\r\n]+)/g, '$1@CODE$2').replace(/(?<!new[ \t]+)Class/g, name),
      moduleFile,
      moduleName,
      external
    );
...
export function getCode(
  ...
  let moduleFile = path.resolve(src, name),
    moduleName = [reName, name].filter(it => it).join('_'),
    code: string = '',
    outName = (fileName || name) + outAddName + '.js';
  reName = reName || name;
  console.log(ly, moduleName);
  return renderModule(moduleFile, reName)
    .then(() => {
