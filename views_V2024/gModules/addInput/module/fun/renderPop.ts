...
import {cols, popContentElem, setCurrentTable, id} from '../var';
...
export function renderPop() {
  let form: string = `<div class="layui-form" style="width:100%;" lay-filter=${id}>`,
    table: string = `<div style="width:100%;overflow: hidden;min-height:260px"><table id="${id}" lay-filter="${id}">`;
  cols.map((it, i) => {
    it.field = it.field || i;
  ...
  window.layui.use(['laydate', 'table', 'form'], function () {
    ...
    popContentElem.find('[layDate]').each((_i, elem) => {
      let id = (elem.dataset.id || 0) as number,
        {type, min, max, format} = cols[id] as TableColDate;
      if (!format) {
        if (type === 'date') {
          format = 'yyyy/MM/dd';
