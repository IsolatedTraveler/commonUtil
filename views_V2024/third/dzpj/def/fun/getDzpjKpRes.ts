 ...
 */
export function getDzpjKpRes(data: any, url: string, bbid?: string, printParam?: DzpjKpPrintParam) {
  // 返回同步和异步不同的处理方式
  let kp = ajaxPost(url, data).then((res) => {
    // 开票成功，判断是否打印收费凭证
    return isPrintPz(bbid, data, dzpjKpIsPrint, '电子票据开票成功，是否打印收费凭证？', printParam).catch(() => null)
  }).catch(() => {
    // 开票失败，提示用户，用户自行选择是否打印收费凭证
    return isPrintPz(bbid, data, dzpjKpIsPrint == '否' ? '否' : '提示', '电子票据开票失败，是否继续打印收费凭证？', printParam).finally(() => {
